...
const allEmployeeQuery = `SELECT e.id, e.first_name AS "First Name", e.last_name AS "Last Name", r.title, d.departments_name AS "Department", IFNULL(r.salary, 'No Data') AS "Salary", CONCAT(m.first_name," ",m.last_name) AS "Manager"
FROM employees e
LEFT JOIN roles r 
ON r.id = e.roles_id 
...
const addEmployeeQuestions = ['First name?', 'Last name?', 'Role?', 'Who is their manager?']
const roleQuery = 'SELECT * from roles; SELECT CONCAT (e.first_name," ",e.last_name) AS full_name FROM employees e'
const mgrQuery = 'SELECT CONCAT (e.first_name," ",e.last_name) AS full_name, r.title, d.departments_name FROM employees e INNER JOIN roles r ON r.id = e.roles_id INNER JOIN departments d ON d.id = r.departments_id WHERE departments_name = "Management";'
        ...
        inquirer.prompt([
            {
                ...
                type: 'list',
                choices: function () {
                    let choiceArray = results.map(choice => choice.departments_name)
                    return choiceArray;
                },
            ...
            for (let i = 0; i < results.length; i++) {
                if (results[i].departments_name === answer.deptChoice) {
                    chosenDept = results[i];
                }
            }

            let query = 'SELECT e.id, e.first_name AS "First Name", e.last_name AS "Last Name", r.title AS "Title", d.departments_name AS "Department", r.salary AS "Salary" FROM employees e INNER JOIN roles r ON r.id = e.roles_id INNER JOIN departments d ON d.id = r.departments_id WHERE ?;';
            connection.query(query, { departments_name: chosenDept.departments_name }, (err, res) => {
                if (err) throw err;
                console.log(' ');
                console.table(chalk.yellow(`All Employees by Department: ${chosenDept.departments_name}`), res)
                startApp();
            })
        })
    })
...
const showByManager = () => {
    connection.query(mgrQuery, (err, results) => {
        ...
        inquirer.prompt([
            ...
            const mgrQuery2 = `SELECT e.id, e.first_name AS "First Name", e.last_name AS "Last Name", IFNULL(r.title, "No Data") AS "Title", IFNULL(d.departments_name, "No Data") AS "Department", IFNULL(r.salary, 'No Data') AS "Salary", CONCAT(m.first_name," ",m.last_name) AS "Manager"
                FROM employees e
                LEFT JOIN roles r 
                ON r.id = e.roles_id 
                LEFT JOIN departments d 
    ...
    connection.query(addRoleQuery, (err, results) => {
        ...
        inquirer.prompt([
            ...
            },
            {
                ...
                type: 'list',
                choices: function () {
                    let choiceArray = results[1].map(choice => choice.departments_name);
                    return choiceArray;
                },
            ...
            connection.query(
                ...
                (SELECT id FROM departments WHERE departments_name = "${answer.dept}"));`
            )
            startApp();
...
const viewDept = () => {
    query = `SELECT departments_name AS "Departments" FROM departments`;
    connection.query(query, (err, results) => {
        if (err) throw err;
...
const addDept = () => {
    ...
    connection.query(query, (err, results) => {
        ...
        inquirer.prompt([
            ...
            connection.query(`INSERT INTO departments(departments_name) VALUES( ? )`, answer.newDept)
            startApp();
        })
    })
}
...
const removeDept = () => {
    ...
    connection.query(query, (err, results) => {
        ...
        inquirer.prompt([
            ...
            connection.query(`DELETE FROM departments WHERE ? `, { departments_name: answer.dept })
            startApp();
        })
    })
